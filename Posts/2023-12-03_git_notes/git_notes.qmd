...
| `git clean -h`     | Quick help                                |
| `git clean -n`     | Dry run                                   |
| `git clean -f`     | Git clean untracked files                 |
| `git clean -fd`    | Git clean untracked files and directories |

- Always run with `-n` first, e.g. `git clean -nfd` then `git clean -fd`
- Goes well with a `git restore .` first if you want to undo everything from the current changes to the working tree
